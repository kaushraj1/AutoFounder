...
_SOCIAL = (
    {"x_thread": ["T1", "T2"], "linkedin_post": "LI post", "show_hn_post": "HN", "status": "draft"},
    60,
)
_EMAIL = (
    {
...
class TestToolFailureDegradation:
    ...
    async def test_dalle_failure_degrades_to_prompt_only(self) -> None:
        ...
        async def fake_dalle(
            prompt, *, size="1024x1024", quality="standard", style="vivid", asset_type="unknown"
        ):
            return {
        ...
        with (
            ...
            patch(
                "app.agents.marketing.nodes.generate_visual_assets.call_llm",
                return_value=_VISUAL_PROMPTS,
            ),
            patch(
