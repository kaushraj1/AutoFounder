...
def _llm_side_effect(prompt: str, **kw) -> tuple:
    ...
    if "brand voice" in prompt.lower():
        return (
            {
                ...
                "seo_keyword_targets": ["saas"],
                "competitor_gaps": ["Gap 1"],
                "target_audience_summary": "Test audience",
            },
            100,
        )
        ...
        return (
            {
                "tagline": "Ship fast",
                "description": "A boilerplate for SaaS founders.",
                "first_comment": "Hey PH!",
                "maker_note": "Built to ship fast.",
                "gallery_captions": ["Cap 1"],
                "topics": ["Dev Tools"],
                "status": "draft",
            },
            150,
        ...
        return (
            {
                "logo": {
                    "asset_type": "logo",
                    "dall_e_prompt": "A logo",
                    "generated_url": None,
                    "status": "pending",
                },
                "og_image": {
                    "asset_type": "og_image",
                    "dall_e_prompt": "OG image",
                    "generated_url": None,
                    "status": "pending",
                },
                "social_card": {
                    "asset_type": "social_card",
                    "dall_e_prompt": "Social",
                    "generated_url": None,
                    "status": "pending",
                },
                "email_banner": {
                    "asset_type": "email_banner",
                    "dall_e_prompt": "Banner",
                    "generated_url": None,
                    "status": "pending",
                },
                "total_generated": 0,
...
class TestGraphHappyPath:
    ...
    async def test_graph_runs_to_completion(self) -> None:
        ...
        with (
            patch(
                "app.agents.marketing.nodes.analyse_brand.call_llm", new_callable=AsyncMock
            ) as mock_brand,
            patch(
                "app.agents.marketing.nodes.generate_landing_page.call_llm", new_callable=AsyncMock
            ) as mock_lp,
            patch(
                "app.agents.marketing.nodes.generate_seo_blogs.call_llm", new_callable=AsyncMock
            ) as mock_blogs,
            patch(
                "app.agents.marketing.nodes.generate_product_hunt_kit.call_llm",
                new_callable=AsyncMock,
            ) as mock_ph,
            patch(
                "app.agents.marketing.nodes.generate_social_posts.call_llm", new_callable=AsyncMock
            ) as mock_social,
            patch(
                "app.agents.marketing.nodes.generate_email_sequences.call_llm",
                new_callable=AsyncMock,
            ) as mock_email,
            patch(
                "app.agents.marketing.nodes.generate_visual_assets.call_llm", new_callable=AsyncMock
            ) as mock_visual,
            patch(
                "app.agents.marketing.nodes.hallucination_check.call_llm", new_callable=AsyncMock
            ) as mock_hall,
            patch(
                "app.agents.marketing.nodes.render_gtm_report.call_llm_text", new_callable=AsyncMock
            ) as mock_gtm,
            patch(
                "app.agents.marketing.nodes.analyse_brand.tavily_search", new_callable=AsyncMock
            ) as mock_tavily,
            patch(
            ...
            patch(
                "app.agents.marketing.nodes.schedule_posts.buffer_schedule", new_callable=AsyncMock
            ) as mock_buffer,
            patch(
                "app.agents.marketing.nodes.schedule_posts.resend_broadcast", new_callable=AsyncMock
            ) as mock_resend,
        ):
            # Configure all mocks
