...
class MarketingFakeLLMRouter:
    ...
    def _analyse_brand_response(self) -> str:
        return json.dumps({
            ...
            "seo_keyword_targets": [
                {"keyword": "nextjs saas boilerplate", "intent": "commercial", "notes": "high volume"},
                {"keyword": "ship saas fast", "intent": "informational", "notes": "growing trend"},
    ...
    def _landing_page_response(self) -> str:
        return json.dumps({
            ...
            "features_section": [
                {"title": "Auth in 5 minutes", "description": "Magic link and Google OAuth pre-configured with Supabase."},
                {"title": "Stripe billing", "description": "Monthly and annual subscription plans, ready to go."},
                {"title": "Email with Resend", "description": "Transactional emails pre-wired and tested."},
            ],
            "social_proof_section": "Join 2,000+ indie hackers who shipped with ShipFast. [PLACEHOLDER]",
            "pricing_section": [
                {"tier_name": "Starter", "price": "$149", "description": "One-time purchase, lifetime updates", "cta": "Buy Now"},
                {"tier_name": "All-Access", "price": "$299", "description": "All templates + future additions", "cta": "Get All-Access"},
            ],
            "faq_section": [
                {"question": "What stack does ShipFast use?", "answer": "Next.js 14 App Router, Supabase, Stripe, and Resend."},
                {"question": "Is this a one-time purchase?", "answer": "Yes — one-time fee, lifetime updates."},
            ],
    ...
    def _product_hunt_response(self) -> str:
        return json.dumps({
            ...
            "gallery_captions": ["Auth in 5 minutes", "Stripe billing pre-wired", "Deploy to Vercel instantly", "Full Next.js 14 App Router"],
...
class MarketingFakeToolRegistry:
    ...
    async def call(self, tool_name: str, args: dict[str, Any]) -> dict[str, Any]:
        ...
        if tool_name == "tavily_search":
            return {"results": [{"title": "Competitor", "url": "https://example.com", "content": "Sample content"}]}
