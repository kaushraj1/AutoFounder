...
async def test_terraform_run_creates_s3_bucket_on_localstack(
    ...
    init = await tools.terraform_run(action="init", working_dir=str(work_dir), vars={})
    assert init["returncode"] == 0, init.get("stderr")
    assert init["ok"] is True

    apply = await tools.terraform_run(action="apply", working_dir=str(work_dir), vars=vars_)
    ...
    # Clean up.
    destroy = await tools.terraform_run(action="destroy", working_dir=str(work_dir), vars=vars_)
