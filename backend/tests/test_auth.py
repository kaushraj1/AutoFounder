...
def test_verify_jwt_nested_app_metadata() -> None:
    ...
    claims = {
        "sub": "user-123",
        "app_metadata": {"organization_id": "org-xyz", "role": "admin", "scopes": ["runs:read"]},
    }
    token = jwt.encode(claims, settings.supabase_jwt_secret, algorithm="HS256")
...
def test_verify_jwt_sub_fallback() -> None:
    settings = get_settings()
    claims = {"sub": "user-uuid-111", "role": "founder"}
    token = jwt.encode(claims, settings.supabase_jwt_secret, algorithm="HS256")
...
def test_verify_mtls_enabled_success() -> None:
    ...
    response = client.get(
        "/",
        headers={"X-SSL-Client-Verify": "SUCCESS", "X-SSL-Client-DN": "CN=orchestrator.internal"},
    )
    assert response.json() == {"ok": True}
...
def test_verify_mtls_enabled_failures() -> None:
    ...
    # Mismatch DN
    response = client.get(
        "/", headers={"X-SSL-Client-Verify": "SUCCESS", "X-SSL-Client-DN": "CN=untrusted.internal"}
    )
    ...
    response = client.get(
        "/",
        headers={"X-SSL-Client-Verify": "FAILED", "X-SSL-Client-DN": "CN=orchestrator.internal"},
    )
    assert response.json() == {"ok": False}
...
def test_get_principal_valid_jwt(test_app: FastAPI, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    claims = {"sub": "user-1122", "organization_id": "org-555", "role": "founder"}
    token = jwt.encode(claims, settings.supabase_jwt_secret, algorithm="HS256")
